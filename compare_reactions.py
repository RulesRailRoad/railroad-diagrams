...
def compare_reactions(expanded_reactants, expanded_products, arrow, mol_site_dict):
    ...
    for part in reactant_parts:
        ...
        if rmol in rmol_counter:    
            rmol_counter[rmol] +=1
        else:
            rmol_counter[rmol] = 1
        mol_label = f'{rmol} #{rmol_counter[rmol]}'

        for site in rsites_parts:
            all_rsites.append((mol_label, site))
    ...
    for part in product_parts:
        ...
        if pmol in pmol_counter:    
            pmol_counter[pmol] +=1
        else:
            pmol_counter[pmol] = 1
        mol_label = f'{pmol} #{pmol_counter[pmol]}'

        for site in psites_parts:
            all_psites.append((mol_label, site))
    ...
    for i in range(len(all_rsites)):
        ...
        if r_raw != p_raw:
            ...
            change = []
            if rstate != pstate:
                rmol_base = rmol.split(" #")[0]
                mole_sites = mol_site_dict[rmol_base]
                for site in mole_sites:
                    if site.startswith(rsite+"~"):
                        ...
                        if state_list.index(rstate.split("~")[1]) <  state_list.index(pstate.split("~")[1]):
                            change.append(state_change_down)
                        elif state_list.index(rstate.split("~")[1]) >  state_list.index(pstate.split("~")[1]):
                            change.append(state_change_up)


            if rbond != pbond:
                if arrow == "->":
                    if rbond == "!-":
                        change.append(bond_added_non_rev)
                    else:
                        change.append(bond_removed_non_rev)
                else:
                    if rbond == "!-":
                        change.append(bond_added_rev)
                    else:
                        change.append(bond_removed_rev)
